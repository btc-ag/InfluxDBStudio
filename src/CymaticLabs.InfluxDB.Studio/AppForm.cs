...
namespace CymaticLabs.InfluxDB.Studio
{
    ...
    public partial class AppForm : Form
    {
        ...
        enum InfluxDbNodeTypes
        {
            ...
            /// <summary>
            /// An InfluxDB database retention policy.
            /// </summary>
            RetentionPolicy = 4,
        }
        ...
        private void disconnectButton_Click(object sender, EventArgs e)
        {
            ...
            switch (type)
            {
                ...
                case InfluxDbNodeTypes.RetentionPolicy:
                    Disconnect(node.Parent.Parent);
                    break;

                case InfluxDbNodeTypes.Measurement:
                    Disconnect(node.Parent.Parent.Parent);
                    break;
            }
        }
        ...
        async Task ExpandNodeChildren(TreeNode node)
        {
            try
            {
                ...
                switch (GetNodeType(node))
                {
                    ...
                    case InfluxDbNodeTypes.Database:
                        await RenderRetentionPolicy(node, client);
                        break;
        ...
        async Task DropMeasurement(TreeNode node)
        {
            ...
            {
                var name = node.Text;
                var dbName = node.Parent.Parent.Text;
        ...
        void NewQuery(TreeNode node)
        {
            try
            {
                ...
                switch (GetNodeType(node))
                {
                    ...
                    // Measurement
                    case InfluxDbNodeTypes.Measurement:
                        database = node.Parent.Parent.Text;
                        retentionPolicy = node.Parent.Text;
                ...
                queryControl.EditorText = string.Format("SELECT * FROM \"{0}\".\"{1}\".\"{2}\" WHERE time > now() - 2h", database, retentionPolicy, measurement);
        ...
        // Renders the Retention Policy unser a database node
        async Task RenderRetentionPolicy(TreeNode node, InfluxDbClient client)
        {
            try
            {
                // this is for my own porpuse
                //string imagePath = @"..\..\Resources\ImagesFinal\Database.png";
                ...
                //render Policies
                foreach(var policyName in retentionPolicies)
                {
                    var policyNode = CreateTreeNode(policyName.Name, InfluxDbNodeTypes.RetentionPolicy);
                    //todo das Kontextmenü für policy erstellen (Wenn vorhanden. Kann man selects nur auf Policy machen? Glaube nicht)
                    //policyNode.ContextMenuStrip = measurementContextMenu;
                    node.Nodes.Add(policyNode);

                    // Don't render measurement, instead include a loading place holder
                    var placeholderNode = CreateTreeNode("loading...", InfluxDbNodeTypes.LoadingPlacholder);
                    policyNode.Nodes.Add(placeholderNode);
